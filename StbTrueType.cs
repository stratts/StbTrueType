...
namespace StbTrueType
{
    public static class Stbtt
    {
        public class Buf
        {
            ...
            public Buf(Memory<byte> data, int size)
            {
                this.data = data;
                this.size = size;
                cursor = 0;
            ...
            public uint get16() => get(2);

            public uint get32() => get(4);
        ...
        public class FontInfo
        {
            ...
            public Buf cff;                    // cff font data
            public Buf charstrings;            // the charstring index
            public Buf gsubrs;                 // global charstring subroutines index
            public Buf subrs;                  // private charstring subroutines index
            public Buf fontdicts;              // array of font dicts
            public Buf fdselect;               // map from glyph to fontdict
        };
        ...
        static bool tag4(Memory<byte> m, char c0, char c1, char c2, char c3)
        {
            var p = m.Span;
            return ((p)[0] == (c0) && (p)[1] == (c1) && (p)[2] == (c2) && (p)[3] == (c3));
        }
        static bool _tag(Memory<byte> p, string str) => tag4(p, str[0], str[1], str[2], str[3]);

        static bool _isfont(Memory<byte> font)
        {
            // check the version number
            if (tag4(font, '1', (char)0, (char)0, (char)0))
                return true; // TrueType 1
            if (_tag(font, "typ1"))
                return true; // TrueType with type 1 font -- we don't support this!
            if (_tag(font, "OTTO"))
                return true; // OpenType with CFF
            if (tag4(font, (char)0, (char)1, (char)0, (char)0))
        ...
        static void _cff_skip_operand(Buf b)
        {
            int v, b0 = b.peek8();
            _assert(b0 >= 28);
            if (b0 == 30)
        ...
        static int _cff_int(Buf b)
        {
            int b0 = b.get8();
            if (b0 >= 32 && b0 <= 246)
                return b0 - 139;
            else if (b0 >= 247 && b0 <= 250)
                return (b0 - 247) * 256 + b.get8() + 108;
            else if (b0 >= 251 && b0 <= 254)
                return -(b0 - 251) * 256 - b.get8() - 108;
            else if (b0 == 28)
                return (int)b.get16();
            else if (b0 == 29)
                return (int)b.get32();
        ...
        static Buf _cff_get_index(Buf b)
        {
        ...
        static Buf _cff_index_get(Buf b, int i)
        {
            ...
            _assert(offsize >= 1 && offsize <= 4);
            b.skip(i * offsize);
            start = (int)b.get(offsize);
            end = (int)b.get(offsize);
            return b.range(2 + (count + 1) * offsize + start, end - start);
        }

        static void _dict_get_int(Buf b, int key, out uint outvar)
        {
            Buf operands = _dict_get(b, key);
            outvar = (uint)_cff_int(operands);
        }

        static void _dict_get_ints(Buf b, int key, int outcount, uint[] outarr)
        {
            int i;
            Buf operands = _dict_get(b, key);
            for (i = 0; i < outcount && operands.cursor < operands.size; i++)
                outarr[i] = (uint)_cff_int(operands);
        }

        static Buf _dict_get(Buf b, int key)
        {
            b.seek(0);
            while (b.cursor < b.size)
            {
        ...
        static Buf _get_subr(Buf idx, int n)
        {
        ...
        static int _cff_index_count(Buf b)
        {
            b.seek(0);
            return (int)b.get16();
        }

        static Buf _get_subrs(Buf cff, Buf fontdict)
        {
            uint subrsoff = 0;
            ...
            if (private_loc[1] == 0 || private_loc[0] == 0)
                return new(null, 0);
            pdict = cff.range((int)private_loc[1], (int)private_loc[0]);
            _dict_get_int(pdict, 19, out subrsoff);
            if (subrsoff == 0)
                return new(null, 0);
        ...
        public static uint _find_table(Memory<byte> data, int fontstart, string tag)
        {
            ...
            for (i = 0; i < num_tables; ++i)
            {
                ...
                if (_tag(data.Slice((int)loc + 0), tag))
        ...
        public static int GetFontOffsetForIndex(Memory<byte> font_collection, int index) =>
            GetFontOffsetForIndex_internal(font_collection, index);

        static int GetFontOffsetForIndex_internal(Memory<byte> font_collection, int index)
        {
        ...
        static int InitFont_internal(FontInfo info, Memory<byte> data, int fontstart)
        {
            uint cmap, t;
            int i, numTables;
            ...
            info.loca = (int)_find_table(data, fontstart, "loca"); // required
            info.head = (int)_find_table(data, fontstart, "head"); // required
            info.glyf = (int)_find_table(data, fontstart, "glyf"); // required
            info.hhea = (int)_find_table(data, fontstart, "hhea"); // required
            info.hmtx = (int)_find_table(data, fontstart, "hmtx"); // required
            info.kern = (int)_find_table(data, fontstart, "kern"); // not required
            info.gpos = (int)_find_table(data, fontstart, "GPOS"); // not required
            ...
            else
            {
                ...
                _dict_get_int(topdict, 17, out charstrings);
                _dict_get_int(topdict, 0x100 | 6, out cstype);
                _dict_get_int(topdict, 0x100 | 36, out fdarrayoff);
                _dict_get_int(topdict, 0x100 | 37, out fdselectoff);
                ...
                if (fdarrayoff != 0)
                {
                    ...
                    info.fdselect = b.range((int)fdselectoff, (int)(b.size - fdselectoff));
                }
        ...
        static int FindGlyphIndex(FontInfo info, int unicode_codepoint)
        {
            ...
            {
                _assert(false); // @TODO: high-byte mapping for japanese/chinese/korean
                return 0;
            }
            else if (format == 4)
        ...
        static int GetGlyphShape(FontInfo info, int glyph_index, out vertex[] pvertices)
        {
            if (info.cff.size == 0)
                return _GetGlyphShapeTT(info, glyph_index, out pvertices);
            else
                throw new NotImplementedException();
            //return _GetGlyphShapeT2(info, glyph_index, pvertices);
        }

        static int _GetGlyphShapeTT(FontInfo info, int glyph_index, out vertex[] pvertices)
        {
            ...
            if (numberOfContours > 0)
            {
                ...
                for (i = 0; i < n; ++i)
                {
                    ...
                    if (next_move == i)
                    {
                        ...
                        setvertex(ref vertices[num_vertices++], STBTT_vmove, sx, sy, 0, 0);
                        was_off = false;
                    ...
                    else
                    {
                        ...
                        {                // if it's a curve
                            if (was_off) // two off-curve control points in a row means interpolate an on-curve midpoint
                                setvertex(ref vertices[num_vertices++], STBTT_vcurve, (cx + x) >> 1, (cy + y) >> 1, cx, cy);
                            cx = x;
                            cy = y;
                            was_off = true;
                        }
                        ...
                        {
                            if (was_off)
                                setvertex(ref vertices[num_vertices++], STBTT_vcurve, x, y, cx, cy);
                            else
                                setvertex(ref vertices[num_vertices++], STBTT_vline, x, y, 0, 0);
                            was_off = false;
                        }
                    }
                }
            ...
            else if (numberOfContours < 0)
            {
                ...
                while (more)
                {
                    ...
                    // Get indexed glyph.
                    comp_num_verts = GetGlyphShape(info, gidx, out comp_verts);
                    if (comp_num_verts > 0)
                    {
                        // Transform vertices.
                        for (i = 0; i < comp_num_verts; ++i)
                        {
                            ref vertex v = ref comp_verts[i];
                            short x, y;
                            x = v.x;
                            y = v.y;
        ...
        static int _close_shape(vertex[] vertices, int num_vertices, bool was_off, bool start_off,
                              int sx, int sy, int scx, int scy, int cx, int cy)
        {
            if (start_off)
            {
                if (was_off)
                    setvertex(ref vertices[num_vertices++], STBTT_vcurve, (cx + scx) >> 1, (cy + scy) >> 1, cx, cy);
                setvertex(ref vertices[num_vertices++], STBTT_vcurve, sx, sy, scx, scy);
            }
            else
            {
                if (was_off)
                    setvertex(ref vertices[num_vertices++], STBTT_vcurve, sx, sy, cx, cy);
                else
                    setvertex(ref vertices[num_vertices++], STBTT_vline, sx, sy, 0, 0);
            }
            return num_vertices;
        }
        ...
        public static Memory<byte> GetCodepointBitmap(FontInfo info, float scale_x, float scale_y, int codepoint, out int width, out int height, out int xoff, out int yoff)
        {
            return GetCodepointBitmapSubpixel(info, scale_x, scale_y, 0.0f, 0.0f, codepoint, out width, out height, out xoff, out yoff);
        }

        static Memory<byte> GetCodepointBitmapSubpixel(FontInfo info, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint, out int width, out int height, out int xoff, out int yoff)
        {
            return GetGlyphBitmapSubpixel(info, scale_x, scale_y, shift_x, shift_y, FindGlyphIndex(info, codepoint), out width, out height, out xoff, out yoff);
        }

        public static void GetCodepointBitmapBoxSubpixel(FontInfo font, int codepoint, float scale_x, float scale_y, float shift_x, float shift_y, out int ix0, out int iy0, out int ix1, out int iy1)
        {
            GetGlyphBitmapBoxSubpixel(font, FindGlyphIndex(font, codepoint), scale_x, scale_y, shift_x, shift_y, out ix0, out iy0, out ix1, out iy1);
        }

        public static void MakeCodepointBitmapSubpixel(FontInfo info, Memory<byte> output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int codepoint)
        {
            MakeGlyphBitmapSubpixel(info, output, out_w, out_h, out_stride, scale_x, scale_y, shift_x, shift_y, FindGlyphIndex(info, codepoint));
        }

        static void MakeGlyphBitmapSubpixel(FontInfo info, Memory<byte> output, int out_w, int out_h, int out_stride, float scale_x, float scale_y, float shift_x, float shift_y, int glyph)
        {
            int ix0, iy0;
            vertex[] vertices;
            int num_verts = GetGlyphShape(info, glyph, out vertices);
            _bitmap gbm;

            GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, shift_x, shift_y, out ix0, out iy0, out _, out _);
            gbm.pixels = output;
            gbm.w = out_w;
            gbm.h = out_h;
            ...
            if (gbm.w != 0 && gbm.h != 0)
                Rasterize(ref gbm, 0.35f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ix0, iy0, true);
        }

        static Memory<byte> GetGlyphBitmapSubpixel(FontInfo info, float scale_x, float scale_y, float shift_x, float shift_y, int glyph, out int width, out int height, out int xoff, out int yoff)
        {
            int ix0, iy0, ix1, iy1;
            _bitmap gbm;
            vertex[] vertices;
            ...
            GetGlyphBitmapBoxSubpixel(info, glyph, scale_x, scale_y, shift_x, shift_y, out ix0, out iy0, out ix1, out iy1);
        ...
        static void GetGlyphBitmapBoxSubpixel(FontInfo font, int glyph, float scale_x, float scale_y, float shift_x, float shift_y, out int ix0, out int iy0, out int ix1, out int iy1)
        {
            int x0 = 0, y0 = 0, x1, y1; // =0 suppresses compiler warning
            if (GetGlyphBox(font, glyph, out x0, out y0, out x1, out y1) == 0)
            {
                // e.g. space character
                ix0 = 0;
                iy0 = 0;
        ...
        static void _track_vertex(ref _csctx c, int x, int y)
        {
        ...
        static void setvertex(ref vertex v, byte type, int x, int y, int cx, int cy)
        {
            v.type = type;
            v.x = (short)x;
            v.y = (short)y;
        ...
        static void _csctx_v(ref _csctx c, uint type, int x, int y, int cx, int cy, int cx1, int cy1)
        {
            if (c.bounds != 0)
            {
                _track_vertex(ref c, x, y);
                if (type == STBTT_vcubic)
                {
                    _track_vertex(ref c, cx, cy);
                    _track_vertex(ref c, cx1, cy1);
                }
            }
            else
            {
                setvertex(ref c.pvertices[c.num_vertices], (byte)type, x, y, cx, cy);
        ...
        static void _csctx_close_shape(ref _csctx ctx)
        {
            if (ctx.first_x != ctx.x || ctx.first_y != ctx.y)
                _csctx_v(ref ctx, STBTT_vline, (int)ctx.first_x, (int)ctx.first_y, 0, 0, 0, 0);
        }

        static void _csctx_rmove_to(ref _csctx ctx, float dx, float dy)
        {
            _csctx_close_shape(ref ctx);
            ctx.first_x = ctx.x = ctx.x + dx;
            ctx.first_y = ctx.y = ctx.y + dy;
            _csctx_v(ref ctx, STBTT_vmove, (int)ctx.x, (int)ctx.y, 0, 0, 0, 0);
        }

        static void _csctx_rline_to(ref _csctx ctx, float dx, float dy)
        {
            ctx.x += dx;
            ctx.y += dy;
            _csctx_v(ref ctx, STBTT_vline, (int)ctx.x, (int)ctx.y, 0, 0, 0, 0);
        }

        static void _csctx_rccurve_to(ref _csctx ctx, float dx1, float dy1, float dx2, float dy2, float dx3, float dy3)
        {
            float cx1 = ctx.x + dx1;
            float cy1 = ctx.y + dy1;
            float cx2 = cx1 + dx2;
            ...
            ctx.x = cx2 + dx3;
            ctx.y = cy2 + dy3;
            _csctx_v(ref ctx, STBTT_vcubic, (int)ctx.x, (int)ctx.y, (int)cx1, (int)cy1, (int)cx2, (int)cy2);
        }

        static int _GetGlyfOffset(FontInfo info, int glyph_index)
        {
            int g1, g2;

            _assert(info.cff.size == 0);
        ...
        static int _GetGlyphInfoT2(FontInfo info, int glyph_index, out int x0, out int y0, out int x1, out int y1)
        {
            _csctx c = new();
            int r = _run_charstring(info, glyph_index, ref c);
            x0 = r != 0 ? c.min_x : 0;
            y0 = r != 0 ? c.min_y : 0;
            x1 = r != 0 ? c.max_x : 0;
            y1 = r != 0 ? c.max_y : 0;
        ...
        static int GetGlyphBox(FontInfo info, int glyph_index, out int x0, out int y0, out int x1, out int y1)
        {
            x0 = 0; y0 = 0; x1 = 0; y1 = 0;
            if (info.cff.size != 0)
            {
                _GetGlyphInfoT2(info, glyph_index, out x0, out y0, out x1, out y1);
            }
            else
            {
                int g = _GetGlyfOffset(info, glyph_index);
        ...
        static int _run_charstring(FontInfo info, int glyph_index, ref _csctx c)
        {
            bool in_header = true;
            ...
            while (b.cursor < b.size)
            {
                ...
                switch (b0)
                {
                    ...
                    case 0x15: // rmoveto
                        ...
                        _csctx_rmove_to(ref c, s[sp - 2], s[sp - 1]);
                        break;
                    case 0x04: // vmoveto
                        in_header = false;
                        if (sp < 1)
                            return STBTT__CSERR("vmoveto stack");
                        _csctx_rmove_to(ref c, 0, s[sp - 1]);
                        break;
                    case 0x16: // hmoveto
                        in_header = false;
                        if (sp < 1)
                            return STBTT__CSERR("hmoveto stack");
                        _csctx_rmove_to(ref c, s[sp - 1], 0);
                        break;
                    ...
                    case 0x05: // rlineto
                        ...
                        for (; i + 1 < sp; i += 2)
                            _csctx_rline_to(ref c, s[i], s[i + 1]);
                        break;
                    ...
                    case 0x1E: // vhcurveto
                        ...
                        for (; ; )
                        {
                            if (!goto_hcurveto)
                            {
                                if (i + 3 >= sp)
                                    break;
                                _csctx_rccurve_to(ref c, 0, s[i], s[i + 1], s[i + 2], s[i + 3], (sp - i == 5) ? s[i + 4] : 0.0f);
                                i += 4;
                            }
                            goto_hcurveto = false;
                            ...
                            if (i + 3 >= sp)
                                break;
                            _csctx_rccurve_to(ref c, s[i], 0, s[i + 1], s[i + 2], (sp - i == 5) ? s[i + 4] : 0.0f, s[i + 3]);
                            i += 4;
                        }
                        break;
                    ...
                    case 0x08: // rrcurveto
                        ...
                        for (; i + 5 < sp; i += 6)
                            _csctx_rccurve_to(ref c, s[i], s[i + 1], s[i + 2], s[i + 3], s[i + 4], s[i + 5]);
                        break;
                    ...
                    case 0x1B: // hhcurveto
                        ...
                        {
                            if (b0 == 0x1B)
                                _csctx_rccurve_to(ref c, s[i], f, s[i + 1], s[i + 2], s[i + 3], 0.0f);
                            else
                                _csctx_rccurve_to(ref c, f, s[i], s[i + 1], s[i + 2], 0.0f, s[i + 3]);
                            f = 0.0f;
                        }
                        break;
                        ...
                        { // two-byte escape
                            ...
                            switch (b1)
                            {
                                ...
                                case 0x22: // hflex
                                    ...
                                    dx5 = s[5];
                                    dx6 = s[6];
                                    _csctx_rccurve_to(ref c, dx1, 0, dx2, dy2, dx3, 0);
                                    _csctx_rccurve_to(ref c, dx4, 0, dx5, -dy2, dx6, 0);
                                    break;
                                ...
                                case 0x23: // flex
                                    ...
                                    dy6 = s[11];
                                    //fd is s[12]
                                    _csctx_rccurve_to(ref c, dx1, dy1, dx2, dy2, dx3, dy3);
                                    _csctx_rccurve_to(ref c, dx4, dy4, dx5, dy5, dx6, dy6);
                                    break;
                                ...
                                case 0x24: // hflex1
                                    ...
                                    dy5 = s[7];
                                    dx6 = s[8];
                                    _csctx_rccurve_to(ref c, dx1, dy1, dx2, dy2, dx3, 0);
                                    _csctx_rccurve_to(ref c, dx4, 0, dx5, dy5, dx6, -(dy1 + dy2 + dy5));
                                    break;
        ...
        static void Rasterize(ref _bitmap result, float flatness_in_pixels, vertex[] vertices, int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, bool invert)
        {
            float scale = scale_x > scale_y ? scale_y : scale_x;
            int winding_count = 0;
            int[] winding_lengths;
            _point[] windings = FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, out winding_lengths, out winding_count);
            if (windings != null)
            {
                _rasterize(ref result, windings, winding_lengths, winding_count, scale_x, scale_y, shift_x, shift_y, x_off, y_off, invert);
            }
        }
        ...
        static int GetGlyphKernAdvance(FontInfo info, int g1, int g2)
        {
            int xAdvance = 0;
        ...
        static int _GetGlyphKernInfoAdvance(FontInfo info, int glyph1, int glyph2)
        {
        ...
        static int _GetGlyphGPOSInfoAdvance(FontInfo info, int glyph1, int glyph2)
        {
            ...
            for (i = 0; i < lookupCount; ++i)
            {
                ...
                for (sti = 0; sti < subTableCount; sti++)
                {
                    ...
                    int coverageIndex = _GetCoverageIndex(table.Slice(coverageOffset), glyph1);
                    if (coverageIndex == -1)
                        continue;
                        ...
                        case 2:
                            {
                                ...
                                { // Support more formats?
                                    ...
                                    int glyph1class = _GetGlyphClass(table.Slice(classDef1Offset), glyph1);
                                    int glyph2class = _GetGlyphClass(table.Slice(classDef2Offset), glyph2);
        ...
        public static int _GetCoverageIndex(Memory<byte> coverageTable, int glyph)
        {
        ...
        static int _GetGlyphClass(Memory<byte> classDefTable, int glyph)
        {
        ...
        public static void GetFontVMetrics(FontInfo info, out int ascent, out int descent, out int lineGap)
        {
        ...
        public static void GetCodepointHMetrics(FontInfo info, int codepoint, out int advanceWidth, out int leftSideBearing)
        {
            GetGlyphHMetrics(info, FindGlyphIndex(info, codepoint), out advanceWidth, out leftSideBearing);
        }

        public static void GetGlyphHMetrics(FontInfo info, int glyph_index, out int advanceWidth, out int leftSideBearing)
        {
        ...
        static _point[] FlattenCurves(vertex[] vertices, int num_verts, float objspace_flatness, out int[] contour_lengths, out int num_contours)
        {
            _point[] points = null;
            int num_points = 0;
                ...
                for (i = 0; i < num_verts; ++i)
                {
                    switch (vertices[i].type)
                    {
                        ...
                        case STBTT_vline:
                            ...
                            break;
                        case STBTT_vcurve:
                            _tesselate_curve(points, ref num_points, x, y,
                            ...
                            break;
                        case STBTT_vcubic:
                            _tesselate_cubic(points, ref num_points, x, y,
                                                   vertices[i].cx, vertices[i].cy,
        ...
        static int _tesselate_curve(_point[] points, ref int num_points, float x0, float y0, float x1, float y1, float x2, float y2, float objspace_flatness_squared, int n)
        {
            // midpoint
            ...
            { // half-pixel error allowed... need to be smaller if AA
                _tesselate_curve(points, ref num_points, x0, y0, (x0 + x1) / 2.0f, (y0 + y1) / 2.0f, mx, my, objspace_flatness_squared, n + 1);
                _tesselate_curve(points, ref num_points, mx, my, (x1 + x2) / 2.0f, (y1 + y2) / 2.0f, x2, y2, objspace_flatness_squared, n + 1);
            }
            else
            {
        ...
        static void _tesselate_cubic(_point[] points, ref int num_points, float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
        {
            ...
            if (flatness_squared > objspace_flatness_squared)
            {
                ...
                _tesselate_cubic(points, ref num_points, x0, y0, x01, y01, xa, ya, mx, my, objspace_flatness_squared, n + 1);
                _tesselate_cubic(points, ref num_points, mx, my, xb, yb, x23, y23, x3, y3, objspace_flatness_squared, n + 1);
            }
            else
            {
        ...
        static Buf _cid_get_glyph_subrs(FontInfo info, int glyph_index)
        {
        ...
        static void _rasterize(ref _bitmap result, _point[] pts, int[] wcount, int windings, float scale_x, float scale_y, float shift_x, float shift_y, int off_x, int off_y, bool invert)
        {
            ...
            e = new _edge[n + 1]; // add an extra one as a sentinel
            n = 0;

            m = 0;
            for (i = 0; i < windings; ++i)
            {
                Span<_point> p = pts.AsSpan().Slice(m);
                m += wcount[i];
                j = wcount[i] - 1;
            ...
            //STBTT_sort(e, n, sizeof(e[0]), _edge_compare);
            _sort_edges(e, n);

            // now, traverse the scanlines and find the intersections on each scanline, use xor winding rule
            _rasterize_sorted_edges(ref result, e, n, vsubsample, off_x, off_y);
        }
        ...
        class _active_edge
        {
            ...
            public _active_edge(_edge e, int off_x, float start_point)
            {
        ...
        static void _rasterize_sorted_edges(ref _bitmap result, Span<_edge> e, int n, int vsubsample, int off_x, int off_y)
        {
            ...
            while (j < result.h)
            {
                ...
                ref _active_edge step = ref active;
                ...
                while (e2.y0 <= scan_y_bottom)
                {
                    if (e2.y0 != e2.y1)
                    {
                        _active_edge z = new(e2, off_x, scan_y_top);
                        if (z != null)
                        {
                            if (j == 0 && off_y != 0)
                            {
                            ...
                            _assert(z.ey >= scan_y_top); // if we get really unlucky a tiny bit of an edge can be out of bounds
                                                         // insert at front
                            z.next = active;
                            active = z;
                        }
                ...
                if (active != null)
                {
                    _fill_active_edges_new(scanline, scanline2, result.w, active, scan_y_top);
                }
        ...
        static void _fill_active_edges_new(Span<float> scanline, Span<float> scanline_fill, int len, _active_edge e, float y_top)
        {
            ...
            while (e != null)
            {
                ...
                if (e.fdx == 0)
                {
                    ...
                    if (x0 < len)
                    {
                        if (x0 >= 0)
                        {
                            ...
                            _handle_clipped_edge(scanline_fill, (int)x0 + 1, e, x0, y_top, x0, y_bottom);
                        }
                        else
                        {
                            _handle_clipped_edge(scanline_fill, 0, e, x0, y_top, x0, y_bottom);
                        }
                    }
                }
                else
                {
                    ...
                    float sy0, sy1;
                    float dy = e.fdy;
                    _assert(e.sy <= y_bottom && e.ey >= y_top);
                    ...
                    if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len)
                    {
                        ...
                        else
                        {
                            ...
                            _assert(Math.Abs(area) <= 1.01f); // accumulated error from area += step unless we round step down
        ...
        static void _handle_clipped_edge(Span<float> scanline, int x, _active_edge e, float x0, float y0, float x1, float y1)
        {
            if (y0 == y1)
                return;
            _assert(y0 < y1);
            ...
            {
                _assert(x0 >= x && x0 <= x + 1 && x1 >= x && x1 <= x + 1);
        ...
        static bool STBTT__COMPARE(_edge a, _edge b) => ((a).y0 < (b).y0);

        static void _sort_edges_quicksort(Span<_edge> p, int n)
        {
            ...
            while (n > 12)
            {
                ...
                {
                    _sort_edges_quicksort(p.Slice(i), n - i);
                    n = j;
                }
            }
        }

        static void _sort_edges_ins_sort(Span<_edge> p, int n)
        {
            int i, j;
            for (i = 1; i < n; ++i)
            {
